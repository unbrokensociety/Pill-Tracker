...
* 📌 **Persistent Island Reminders**: A reminder arrives as a normal heads-up and then stays pinned in the status-bar island with "Taken" and "Snooze" buttons until the dose is actually taken — pressing "Taken" logs the intake and decrements stock right from the notification. The pinned card carries a live elapsed-time chronometer, so vendor islands pick it up as a focus notification: on Xiaomi HyperOS turn on "Focus notifications" for the Medication Notifications channel in system settings and the reminder appears in the top island as a live timer; Samsung One UI 6.1.1+ renders it as a Live Notification. Switchable ("Keep in the island").
...
* 🧭 **Guided First Launch**: Three quick swipeable intro slides — what the app does, a live preview of the reminder island with its «Taken»/«Snooze» buttons, and privacy. The notification permission is requested at the right moment: on the last slide, after the user understands why reminders need it (never as a cold system dialog at startup, and never twice). Then two coach-marks over the real interface with a light scrim so the app stays visible: where today's doses live and the "+" button. The tour always ends with an action — «Add now» opens the real add-medication form. «Skip» always on top, replay from the «Show the guide again» card in Settings. Localized EN/UK/RU.
...
* 🧭 **Навчання при першому вході**: Три короткі слайди, що гортаються, — що вміє застосунок, живий макет острівця-нагадування з кнопками «Прийнято»/«Відкласти» та приватність. Дозвіл на сповіщення просять вчасно: на останньому слайді, коли вже зрозуміло, навіщо він (не системним діалогом на холодному старті й ніколи — двічі). Далі — два coach-марки поверх справжнього інтерфейсу з легким затемненням, тож застосунок лишається видно: де живуть прийоми дня і кнопка «+». Тур завжди завершується дією — «Додати зараз» відкриває справжню форму додавання ліків. «Пропустити» завжди зверху, повтор — карткою «Показати навчання знову» в Налаштуваннях. Переклади EN/UK/RU.
...
* 🧭 **Обучение при первом входе**: Три коротких свайпаемых слайда — что умеет приложение, живой макет островка-напоминания с кнопками «Принято»/«Отложить» и приватность. Разрешение на уведомления запрашивают вовремя: на последнем слайде, когда уже понятно, зачем оно (не системным диалогом на холодном старте и никогда — дважды). Дальше — два coach-маркера поверх настоящего интерфейса с лёгким затемнением, так что приложение остаётся видно: где живут приёмы дня и кнопка «+». Тур всегда завершается действием — «Добавить сейчас» открывает настоящую форму добавления лекарств. «Пропустить» всегда сверху, повтор — карточкой «Показать обучение снова» в Настройках. Переводы EN/UK/RU.
...
### 🇬🇧 English
- **One channel**: always install and update from **GitHub Releases → latest release → `pill-tracker.apk`**. Every release is newer than the previous one (versionCode = `2420 + build number` — strictly growing), so Android always accepts it as an in-place update. The app can also update itself in place — see «In-App Updates» above.
...
### ru Русский
- **Один канал**: устанавливайте и обновляйте приложение только из **GitHub Releases → последний релиз → `pill-tracker.apk`**. Каждый новый релиз выше предыдущего (versionCode = `2420 + номер сборки` — растёт строго), поэтому Android всегда принимает его как обновление поверх установленного. Приложение умеет обновляться ещё и само — см. «Обновления прямо из приложения» выше.
...
This repository includes a full **GitHub Actions CI/CD pipeline** (`.github/workflows/android.yml`) that automatically builds and signs the APK on every commit. The `versionCode` is computed at build time from a strictly growing formula (`2420 + run number`) defined in `app_build.gradle.kts`.
