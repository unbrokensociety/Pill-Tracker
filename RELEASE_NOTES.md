...
### Pill Tracker v2.4.0 — персональне привітання, навчання та дрібний ремонт

* Вітання за часом доби тепер живе за новими правилами: ранок до 10:00, далі день (вечір із 17:00, ніч із 22:00) — і його можна персоналізувати: додайте ім'я при першому вході або олівцем біля привітання
* Оновлення навчання: 7 кроків (тепер і Налаштування), необов'язкове ім'я на екрані вітання, виправлена кнопка «назад» із екрана додавання
* Діалог «оновлень немає» більше не замикає: хрестик, кнопка ОК, системне «назад» і тап по затемненню — все закриває
* Політика конфіденційності та Умови використання переписані: кнопка «Зрозуміло» завжди на екрані, тексти чесно описують мережеву активність апдейтера
* Українська локалізація вичищена: єдиний стиль звернення, типографські апострофи, додане нічне привітання

---

### 👋 Персональне привітання
* **Нові діапазони часу:** «Доброго ранку» — до 10:00 (раніше «ранок» тривав до півдня), «Добрий день» — 10:00–16:59, «Добрий вечір» — 17:00–21:59, «Доброї ночі» — 22:00–03:59 (нічного привітання взагалі не було).
* **Ім'я (необов'язково):** на екрані вітання можна ввести ім'я — воно зберігається тільки локально й з'являється у привітанні («Доброго ранку, Богдане»). Змінити/прибрати його можна маленьким олівцем поруч із привітанням на головному екрані.
* **Привітання та дата більше не «застигають»:** годинник живий — оновлюється щохвилини на межі хвилин, тож застосунок, що залишився відкритим на ніч, вранці сам переїде на нову дату й правильне вітання.

### 🧭 Навчання — серйозне оновлення
* **Новий 7-й крок — Налаштування:** тур тепер проходить усі чотири сторінки, включно з показом, де тема, мова, сповіщення, перевірка оновлень і повтор навчання.
* **Ім'я прямо у вітанні туру** — необов'язкове поле під буллетами: заповнив — і привітання одразу персональне.
* **Виправлений «назад» у турі:** із останнього кроку (екран додавання) системна кнопка «назад» раніше не закривала екран додавання — тур повертався на крок «+», а екран залишався висіти. Тепер крок коректно повертає на пейджер.
* **Екран вітання захищений від малих висот:** картка обмежена по висоті й скролиться зсередини (ландшафт/компактні екрани більше не обрізають кнопку «Почати тур»).

### 🔓 Діалог оновлень: вихід завжди є
* **Корінь скарги «перевірив — оновлень немає, а вийти не можна»:** у стані «актуальна версія» не було жодної кнопки закриття, а таймер автозакриття — єдиний вихід. Тепер: хрестик у шапці (усі стани, включно з «перевіряємо…» при завислій мережі), кнопка «Гаразд», системний «назад» і тап по затемненню — все закриває картку.
* Автозакриття «актуальної версії» пришвидшене до 2.2 с і скасовується будь-якою дією користувача.

### 🛡 Політика та Умови — нова редакція
* **Кнопка «Понятно» більше не тікає під екран:** старі діалоги вручну рахували системні інсети у повноекранному вікні (decorFitsSystemWindows=false), і на деяких пристроях кнопка виходила за межі екрану. Нові діалоги — платформові: шапка з хрестиком, скрол у середині, кнопка приклеєна до низу картки, висота обмежена 90% екрану.
* **Тексти приведені до реальності:** застосунок від v2.3 має інтернет для оновлень — політика тепер прямо говорить, що мережа використовується винятково для GitHub-оновлень, розкриває кожен дозвіл Android, згадує вимкнений хмарний бекап (allowBackup=false) і права за 2297-VI/GDPR. 8 розділів замість 5.

### 🧹 Пам'ять і чистота
* Очистка після оновлення розширена: крім APK у cacheDir/updates, видаляються надбання v2.3.0 у external files, загублені .apk/.tmp у корені кешу й старий каталог Download — після переходу на v2.4.0 на диску не лишається нічого зайвого.
* Хвилинний тік годинника вирівняний по межі хвилин — менше марудних пробудень CPU при відкритому додатку.

### 🇺🇦 Українська локалізація
* Єдиний стиль звернення («ви»), типографські апострофи (’, не ASCII-лапки), виправлені «Твої ліки» → «Ваші ліки», неформальні «перевір/дозволь/повернися» → «перевірте/дозвольте/поверніться», нове нічне привітання й низка дрібних правок фраз.
