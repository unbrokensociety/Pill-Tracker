...
object ReportExportHelper {
    ...
    fun generateAndShareReport(
        ...
        val dosesTodayText = if (isUk) "прийомів за сьогодні" else "doses today"
        val daysText = if (isUk) "дн." else "days"
        val timesPerDayText = if (isUk) "раз(ів) на день" else "time(s) per day"
        val appFooterText = if (isUk) "Сформовано PillTracker Mobile Health App" else "Generated by PillTracker Mobile Health App"
        ...
        builder.append("$adherenceLabel   : $adherenceRate% ($takenCount/$totalScheduled $dosesTodayText)\n")
        builder.append("$streakLabel: $streakDays $daysText\n\n")
        ...
        if (medications.isEmpty()) {
            ...
            medications.forEachIndexed { index, med ->
                ...
                builder.append("    ${if (isUk) "Частота" else "Daily Frequency"} : ${med.timesPerDay} $timesPerDayText\n")
